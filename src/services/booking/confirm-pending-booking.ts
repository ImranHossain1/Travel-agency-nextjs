...
export const confirmAllPendingBookings = async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.log("All pending bookings have been confirmed successfully.");
    } else {
      // Handle error cases, e.g., if the server returns an error response
      console.error("Error pending bookings:", response.statusText);
    }
    ...
    console.error("Error Pending pending bookings:", error);
  }
};
